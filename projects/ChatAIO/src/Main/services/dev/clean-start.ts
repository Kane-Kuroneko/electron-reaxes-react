...
const spawnCleanupHelper = (userDataPath:string) => {
	const scriptPath = path.join( app.getPath( 'temp' ) , `chataio-clean-start-${ Date.now() }.cjs` );
...
const getCleanStartMarkerPath = (userDataPath:string) => {
	return path.join( path.dirname( userDataPath ) , `.chataio-clean-start-${ path.basename( userDataPath ) }.json` );
};
