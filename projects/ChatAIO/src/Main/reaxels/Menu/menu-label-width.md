...
`menu-label-width.ts` 用于规整 ChatAIO 主窗口原生菜单栏里的 `Previous` / `Next` 顶层菜单标签，使其中的 AI 名称部分尽量保持固定视觉宽度。
...
ChatAIO 的 `Previous` / `Next` 是 Electron main thread 中的原生菜单项，入口在 `src/Main/reaxels/Menu/index.ts`。菜单通过 `Menu.buildFromTemplate()` 创建，最终交给宿主系统绘制。
...
- 能完整容纳默认配置中较长的 `ChatAIO (Proxy Test)`。
...
import {
   ...
   getMenuTextWidth ,
   MENU_AI_NAME_TARGET_WIDTH,
} from './projects/ChatAIO/src/Main/reaxels/Menu/menu-label-width.ts';
...
- `ChatAIO (Proxy Test)`：补齐后约 `108.06px`
...
- 调整 `MENU_AI_NAME_TARGET_WIDTH` 时，至少验证 `Claude`、`Grok`、`ChatAIO (Proxy Test)`、`Perplexity-Anselmddddddddddddd`。
- 如果未来主要运行平台切换到 macOS 或 Linux，应重新采样对应平台的菜单字体宽度。
- 如果新增默认 AI 名称明显长于 `ChatAIO (Proxy Test)`，需要重新评估目标宽度。
