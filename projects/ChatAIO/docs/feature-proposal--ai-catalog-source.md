...
当前进度：**批次 0–5 均已落地并确认。** 收尾已做：现行规范只留 `ai-config.md`，本文改为历史记录。

下面「怎么执行 / 硬闸」是当时的施工流程，不再约束后续改动。

| 批次 | 层 | 交付 | 依赖 | 状态 |
|------|----|------|------|------|
| 0 | docs | 本文 + 索引 | 无 | 已确认（目标模型已按供应商目录重写；旧「catalog = 全字段 AIItem」否决） |
| 1 | data + main 加载 | JSON 搬家、瘦 schema、main 用 fs 读 bundled | 0 | 已确认 |
| 2 | main + renderer 读路径 | 删并行 URL 表；renderer 不再 import 目录 | 1 | 已确认（官方 URL 在目录行上，不是派生 Map） |
| 3 | main 纯函数 | 供应商目录校验 + 目录行→种子实例 merge + region + 单测 | 2 | 已确认 |
| 4 | main 安全 + 脚本 | Ed25519 验签模块 + 发布脚本（payload = 瘦目录） | 3 | 已确认 |
| 5 | ipc + Settings UI | 手动检查、预览、确认写入 | 3 + 4 | 已确认 |
...
**状态：已确认**（含方向纠偏：校验供应商目录，merge 是目录行 → 种子实例）  
**层**：main 纯函数 + tests  
...
收尾（2026-09-03）：用户确认以现码为准。本文标已落地；现行规范迁入 `ai-config.md`，手动更新契约留在 `ai-catalog-manual-update.md`。
...
- 现行运行时以 [`ai-config.md`](./architecture/ai-config.md) 为准。
- 手动检查更新以 [`features/ai-catalog-manual-update.md`](./features/ai-catalog-manual-update.md) 为准。
