...
- 用户已经在 ChatAIO 中为不同 AI page 配置不同 proxy，希望某些页面只在“出口 IP 看起来安全”时加载。
...
```text
projects/ChatAIO/src/Main/services/sensitive-region-access/index.ts
```
...
- 增加全局“敏感地区列表”高级设置。
- ~~按 AI family 提供 provider-specific 默认列表。~~ **已由 catalog.region 承担**（见 [`../feature-proposal--ai-catalog-source.md`](../feature-proposal--ai-catalog-source.md)）。
