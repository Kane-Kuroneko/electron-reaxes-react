...
| 层 | 测什么 | 怎么驱动 | 现有锚点 |
|----|--------|----------|----------|
| 纯函数 | dirty 指纹、单条提交不洗净表 | Node `yarn test:ai-order` | `tests/settings-dirty-scopes.test.ts` |
...
4. `https://chatgpt.com/` 也会在 `windows()` 里，**禁止**对远程 AI Page 做 locator。
5. 调 `applySettings` / `applyAIs` 探针前等 `runtimeViewsReady`（`kind==='main'` 在 Phase 0 就真了）。
...
产品不变量见 [`manage-ais-save-scopes.md`](./manage-ais-save-scopes.md)。下面只列「还没自动化」的手势，设计用例时按分层表选 DOM 或探针，不要一条里既当单元又当手势。

| 手势 / 不变量 | 建议层 | 通过条件（摘要） |
|---------------|--------|------------------|
| 弹窗 Save 改名 | DOM + 探针 | 表底 Save 仍灭；探针读盘已是新名；`disabled` 未被弹窗改掉 |
| 弹窗 Cancel | DOM | store / 磁盘都不变 |
| 页脚改主题 | DOM | 页脚 Apply 亮、表底 Save 灭 |
| 表底 Undo | DOM | 只丢表草稿，页脚 dirty 仍在 |
| 页脚 Discard | DOM | 不丢表草稿 |
| Startup AI Page 单选 | DOM | 走页脚 dirty，不点亮表底 |
