...
PromptView 是 ChatAIO 主窗口内的本地工具侧栏，用两个 `WebContentsView` 分别承载左侧 `PromptViewLeft` 和右侧 `PromptViewRight`。它不属于某个 AI page，也不加载远程页面；其 prompt 数据按左右侧分别持久化，并可在不同 AI page 之间复用。
...
- ChatAIO 已经使用 `mainWindow.contentView.addChildView(view)` 和 `WebContentsView.setBounds()` 管理 AI page 与 SettingsView，本功能沿用该模型。
