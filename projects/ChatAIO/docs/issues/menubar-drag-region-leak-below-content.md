...
| 活动中心 view mount | 唯一入口 `presentActiveCenterView(intent)`：`switch` 才允许 Darwin remount / Win reorder；`recover`（回前台）禁止 reorder。见 [`ai-view-foreground-white-flash.md`](./ai-view-foreground-white-flash.md) |
