...
| **cloneForIPC** | ✅ 主进程 `createMenuData()` 首次下发为 plain JSON；**进入 `reaxel_MainView.store` 后变为 observable**。凡 Renderer → Main（`openDropdownView`、`menuViewAction` 等）及主进程转发 DropdownView 前，**必须** `cloneForIPC`。详见 `.agents/rules/ipc-coding.md`「Store 往返」 |
...
| **拖拽 (titleBarStyle)** | `app-region: drag` 吞 pointer；主壳全窗时 drag 会叠进内容区（#41002） | menubar 空白可拖、按钮 no-drag；主壳 View 裁到 menubar 高度（`clipMainShellToMenuBar`）；禁止 inset:0 overlay；禁止靠砍整栏 drag「假修」 |
...
- Electron draggable region 文档强调 `app-region: drag` 会忽略鼠标点击。MenuView 不手写 `setPosition` 拖窗；栏空白保持 drag、按钮 no-drag。内容区幽灵拖拽靠裁主壳 View（见 `menubar-drag-region-leak-below-content.md`），不要砍 menubar drag。
