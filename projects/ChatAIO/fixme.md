...
本文基于对 `projects/ChatAIO` 源码、文档、构建配置、`.qoder` / `.codex` agent 规则的阅读整理。每个条目尽量保持内聚，目标是可以独立修复、独立验证。
...
AI 页面创建时，`projects/ChatAIO/src/Main/reaxels/Views/utils/initWebContentsView.ts` 会通过 `webPreferences.additionalArguments` 把语言和主题传给 `projects/ChatAIO/src/ai-page-preload.ts`。

设置变更后，`projects/ChatAIO/src/Main/reaxels/Views/AI-Views/index.ts` 的 `updateRuntimeAIView` 会更新 session header、proxy、appearance key，并在变化时 reload。但 `additionalArguments` 是 WebContents 创建时参数，reload 不会重新生成这些 preload 参数。

### 为什么有问题以及后果

业务上，ChatAIO 希望统一控制 AI 页面的语言、主题和 Accept-Language，让不同 AI 服务在一个宿主中尽量保持一致体验。当前实现只能保证部分 session/header 更新，不能保证已创建 AI 页面的 preload 环境也更新。
...
- `projects/ChatAIO/src/Main/reaxels/Views/AI-Views/index.ts`
- `projects/ChatAIO/src/Main/reaxels/Views/utils/initWebContentsView.ts`
- `projects/ChatAIO/src/ai-page-preload.ts`
- 可能涉及 `projects/ChatAIO/src/Main/services/appearance/index.ts`
...
4. `ai-page-preload.ts` 通过 typed main-to-renderer IPC 监听 `ai-page-environment-change`，只在 preload 内部更新：
   ...
   - `document.documentElement.dataset.chataioTheme`
   - `document.documentElement.dataset.chataioThemeSource`
...
`projects/ChatAIO/src/Main/reaxels/Views/utils/initWebContentsView.ts` 只在 `setWindowOpenHandler` 中处理新窗口打开：
...
AI 页面是远程网页，ChatAIO 作为 Electron 宿主需要明确区分“AI 服务内部导航”和“外部链接”。业务目标是让用户在 AI 服务内正常使用，同时防止外部网站、下载页、登录跳转或非预期协议污染 AI WebContents。
...
- `projects/ChatAIO/src/Main/reaxels/Views/utils/initWebContentsView.ts`
- 可能新增工具文件：`projects/ChatAIO/src/Main/reaxels/Views/utils/navigation-policy.ts`
...
`projects/ChatAIO/src/Main/reaxels/Views/AI-Views/index.ts` 的 `destroyAllAndClearData` 从当前 `store.AIViews` 收集 partitions，再逐个 `clearStorageData`、`clearCache`、`clearAuthCache`。

Settings 的 `reset-ais-to-defaults` RPC 位于 `projects/ChatAIO/src/Main/reaxels/Settings/index.ts`，调用该方法后再 reset AI 配置。
...
- `projects/ChatAIO/src/Main/reaxels/Views/AI-Views/index.ts`
- `projects/ChatAIO/src/Main/reaxels/Settings/index.ts`
- 可能涉及 `projects/ChatAIO/src/Main/services/settings/ai-config-service.ts`
...
`projects/ChatAIO/src/Main/reaxels/Views/AI-Views/crash-reporter.ts` 中初始化日志 header 的语句形态为：
...
ChatAIO 承载多个远程 AI 页面，WebContents crash 是需要长期记录和定位的运行时问题。crash reporter 如果日志内容不完整或路径不稳定，会降低线上排查能力。
...
- `projects/ChatAIO/src/Main/reaxels/Views/AI-Views/crash-reporter.ts`
- 可能涉及 `projects/ChatAIO/logs/webview-crashes.md`
...
`projects/ChatAIO/src/Main/reaxels/Settings/index.ts` 的 `submit-settings` RPC 接收 path 和 data，然后直接调用 `applyPatchByPath` 修改当前 settings。
...
- `projects/ChatAIO/src/Main/reaxels/Settings/index.ts`
- `projects/ChatAIO/src/Types/SettingsTypes/SettingsPatchPath.d.ts`
- 可能新增：`projects/ChatAIO/src/Main/services/settings/settings-schema.ts`
...
`projects/ChatAIO/src/Main/services/settings/proxy-service.ts` 的代理测试会临时注册 `app.on('login')`，用于处理 proxy auth。AI view 运行时也会给 `view.webContents` 安装 login handler。
...
- `projects/ChatAIO/src/Main/services/settings/proxy-service.ts`
- `projects/ChatAIO/src/Types/SettingsTypes/NetworkProxy.d.ts`
...
**状态：已被提案批次 2 消化（本地并行 URL 表已删除）；2026-08-28 方向纠偏后目录是扁平供应商列表，不是默认实例袋。** 远程目录 / 签名仍见 [`docs/feature-proposal--ai-catalog-source.md`](./docs/feature-proposal--ai-catalog-source.md) 批次 4–5，本条不再作为独立待办。
...
ChatAIO 的核心业务是管理多个 AI 服务页面。默认 AI 是菜单、首次引导、设置页、session partition、proxy bypass、preload 策略的共同基础。多处数据源会让新增、删除或修改 AI 服务时产生不一致。
...
**状态：文档已按批次 3 与代码对齐（整表 + deletedIds，不是 delta）。** 实现不改成 delta，见 [`docs/feature-proposal--ai-catalog-source.md`](./docs/feature-proposal--ai-catalog-source.md)。
...
- `projects/ChatAIO/docs/architecture/ai-config.md`
- `projects/ChatAIO/src/Main/services/settings/ai-config-service.ts`
- `projects/ChatAIO/src/Types/SettingsTypes/AI.d.ts`
- `projects/ChatAIO/src/Views/SettingsView/reaxels/settings-view/index.ts`
...
## P2-03 preload AI 配置仍以 family 为粒度，可能丢失实例身份 ✅ RESOLVED

### 问题现状
~~`projects/ChatAIO/src/Main/services/settings/ai-config-service.ts` 的 `getPreloadAIFamilies()` 返回去重后的 `AI.AIFamily[]`。~~

### 已完成的修复

1. **新增 `getPreloadAIIds(): string[]`** 替代旧 API，按 AI 实例 ID 粒度返回预加载列表。
2. **IPC schema** `get-preload-ai-families` 返回类型改为 `string[]`。
3. **preload.ts** 导出重命名为 `getPreloadAIIds`。
4. **SettingsView services** 包装函数同步更新。
5. **Main Settings handler** 调用 `getPreloadAIIds()`。
6. **旧 `getPreloadAIFamilies()`** 标记 `@deprecated` 保留为兼容层。
7. **ManageAIs Table 新增 "Preload on Startup" 列**，用户可直接在表格行通过 Switch 开关 toggle `preloadOnStartup`，无需进入 Edit Modal。当 `startupAIPageLoadMode === 'first-ai'` 且为列表第一项时开关强制启用并禁用。
...
- 表格行 Switch 直接 toggle，Apply/Save 后重启生效。

## P2-04 SettingsView 存在无条件启用的 Dev rehancer

### 问题现状

`projects/ChatAIO/src/Views/SettingsView/reaxels/settings-view/index.ts` 无条件执行 `rehancer_Dev`。

`projects/ChatAIO/src/Views/SettingsView/reaxels/settings-view/rehancer_Dev/index.ts` 会把 SettingsView 默认菜单设置为 `net`。
...
- `projects/ChatAIO/src/Views/SettingsView/reaxels/settings-view/index.ts`
- `projects/ChatAIO/src/Views/SettingsView/reaxels/settings-view/rehancer_Dev/index.ts`
...
- `projects/ChatAIO/src/Views/SettingsView/components/Appearance/index.tsx`
- `projects/ChatAIO/src/Views/SettingsView/components/System/index.tsx`
...
- `projects/ChatAIO/src/Views/SettingsView/App.tsx`
- `projects/ChatAIO/src/Views/SettingsView/reaxels/settings-view/index.ts`
- `projects/ChatAIO/src/Views/SettingsView/components/General/index.tsx`
- `projects/ChatAIO/src/Views/SettingsView/components/Appearance/index.tsx`
- `projects/ChatAIO/src/Views/SettingsView/components/System/index.tsx`
...
`projects/ChatAIO/src/Views/SettingsView/reaxels/settings-view/index.ts` 同时承担：
...
SettingsView 是 ChatAIO 复杂度最高的本地 UI。所有业务动作集中在一个 reaxel，会让每次修改都容易影响无关区域。
...
- `projects/ChatAIO/src/Views/SettingsView/reaxels/settings-view/index.ts`
- 可能新增：
  - `projects/ChatAIO/src/Views/SettingsView/reaxels/settings-view/settings-dirty.ts`
  - `projects/ChatAIO/src/Views/SettingsView/reaxels/settings-view/ai-editor.ts`
  - `projects/ChatAIO/src/Views/SettingsView/reaxels/settings-view/proxy-editor.ts`
  - `projects/ChatAIO/src/Views/SettingsView/reaxels/settings-view/appearance-preview.ts`
...
- `projects/ChatAIO/src/Views/SettingsView/reaxels/i18n`
- `projects/ChatAIO/src/Views/GuidingView/reaxels/i18n`
...
ChatAIO 已经支持多语言，且 main menu、tray、SettingsView、GuidingView 都需要语言一致。资源分散和无键级校验会增加漏翻译概率。
...
- `projects/ChatAIO/src/Views/SettingsView/reaxels/i18n/index.ts`
- `projects/ChatAIO/src/Views/SettingsView/reaxels/i18n/langs/*.ts`
- `projects/ChatAIO/src/Views/GuidingView/reaxels/i18n/index.ts`
- `projects/ChatAIO/src/Views/GuidingView/reaxels/i18n/langs/*.ts`
- `projects/ChatAIO/src/Views/SettingsView/components/ManageAIs/index.tsx`
- `projects/ChatAIO/src/Views/GuidingView/components/*.tsx`
- `projects/ChatAIO/docs/architecture/i18n.md`
...
GuidingView 会调用 `guiding-save-progress` 保存部分进度，main 侧在 `projects/ChatAIO/src/Main/reaxels/Views/Guiding-View/index.ts` 处理 progress。

但 renderer 初始化时 `get-guiding-defaults` 返回的是默认配置和环境，未看到把之前保存的 partial progress 合并回 UI 的流程。

### 为什么有问题以及后果

首次引导是用户第一次建立 ChatAIO 运行配置的关键流程。如果用户中途关闭窗口或 app，已选择的语言、网络状态、自定义 AI 是否恢复需要明确。
...
- `projects/ChatAIO/src/Main/reaxels/Views/Guiding-View/index.ts`
- `projects/ChatAIO/src/Views/GuidingView/reaxels/guiding-view/index.ts`
- `projects/ChatAIO/src/Types/Guiding.d.ts`
- `projects/ChatAIO/src/Views/GuidingView/components/*.tsx`
...
SettingsView 的 per-AI proxy auth 在 `projects/ChatAIO/src/Views/SettingsView/components/ManageAIs/index.tsx` 使用 `Input.Password`。

但全局代理服务器编辑弹窗位于 `projects/ChatAIO/src/Views/SettingsView/components/Network/ProxyServers/index.tsx`，其中 password 字段使用普通 `Input`。
...
- `projects/ChatAIO/src/Views/SettingsView/components/Network/ProxyServers/index.tsx`
- 可能涉及 `projects/ChatAIO/src/Views/SettingsView/components/ManageAIs/index.tsx`
...
- `projects/ChatAIO/src/Main/reaxels/Menu/index.ts`
- `projects/ChatAIO/src/Main/reaxels/Views/index.ts`
- 可能新增：`projects/ChatAIO/src/Main/services/settings/runtime-settings.ts`
...
`projects/ChatAIO/src/Main/foundation/electron.conf.ts` 在 `__DEV__` 下开启：
...
ChatAIO 会加载远程 AI 服务网页。Electron 应用承载远程网页时，应尽量收紧 webPreferences 和调试入口。当前配置已有基本隔离，但仍可进一步加强。
...
- `projects/ChatAIO/src/Main/mainWindow.ts`
- `projects/ChatAIO/src/Main/reaxels/Views/utils/initWebContentsView.ts`
- `projects/ChatAIO/src/Main/reaxels/Views/Guiding-View/index.ts`
- `projects/ChatAIO/src/Main/reaxels/Views/FloatingView/index.ts`
- `projects/ChatAIO/src/Main/foundation/electron.conf.ts`
- `projects/ChatAIO/partial.webpack-conf.ts`
...
ChatAIO 四个 tsconfig 都关闭了多项严格检查：
...
ChatAIO 的业务对象嵌套较深，例如 settings、AI item、proxy config、guiding progress。低严格度会让 null、undefined、错误字段名、错误事件参数更容易进入运行时。
...
- `projects/ChatAIO/tsconfig.json`
- `projects/ChatAIO/src/Views/SettingsView/tsconfig.json`
- `projects/ChatAIO/src/Views/GuidingView/tsconfig.json`
- `projects/ChatAIO/src/Views/FloatingView/tsconfig.json`
...
在 `projects/ChatAIO` 下未发现 `*test*` 或 `*spec*` 测试文件。当前质量验证主要依赖手动阅读和运行构建。

### 为什么有问题以及后果

ChatAIO 的高风险逻辑很多是纯业务规则，适合测试：
...
- `projects/ChatAIO/package.json`
- 可能新增 `projects/ChatAIO/src/**/*.test.ts`
- 可能新增 `projects/ChatAIO/test` 或 `projects/ChatAIO/src/test-utils`
...
- `projects/ChatAIO/src/Views/SettingsView/components/Network/GlobalNetProxy/index.tsx`
- `projects/ChatAIO/src/Main/reaxels/Menu/index.ts`
- `projects/ChatAIO/src/Main/reaxels/Settings/index.ts`
- `projects/ChatAIO/src/Main/reaxels/I18n/index.ts`
...
- `projects/ChatAIO/src/Views/SettingsView/App.tsx`
- `projects/ChatAIO/src/Views/SettingsView/components/ManageAIs/index.tsx`
- `projects/ChatAIO/src/Views/GuidingView/components/LongPressButton/index.tsx`
...
- `projects/ChatAIO/src/Views/SettingsView/components/ManageAIs/index.tsx`
- `projects/ChatAIO/src/Views/GuidingView/components/LongPressButton/index.tsx`
- 可能新增 `projects/ChatAIO/src/Views/shared/components/LongPressButton.tsx`
...
`projects/ChatAIO/src/shared/structs/settings/index.ts` 仍保留旧的 settings/reaxable 结构和 `Menus` 类型。

当前更权威的设置类型在：

- `projects/ChatAIO/src/Types/SettingsTypes/index.d.ts`
- `projects/ChatAIO/src/Types/SettingsTypes/AI.d.ts`
- `projects/ChatAIO/src/Types/SettingsTypes/NetworkProxy.d.ts`
...
- `projects/ChatAIO/src/shared/structs/settings/index.ts`
- `projects/ChatAIO/src/Types/SettingsTypes/*.d.ts`
- `projects/ChatAIO/src/Views/SettingsView/reaxels/settings-view/index.ts`
...
- `projects/ChatAIO/tsconfig.json`
- 根 `.gitignore`
- `projects/ChatAIO/logs/webview-crashes.md`
- `projects/ChatAIO/src/Main/reaxels/Views/AI-Views/crash-reporter.ts`
...
ChatAIO docs 已覆盖 AI config、appearance/theme、i18n 和若干 issues，但部分内容与实现存在偏差，例如 AI config 的用户配置模型。

`projects/ChatAIO/todo.md` 中的外链逻辑、proxy 优先级、tray、i18n、快捷键等事项，有些已部分实现，有些仍未完成。

### 为什么有问题以及后果

ChatAIO 是多模块 Electron app，后续 agent 或开发者很依赖文档理解架构。如果文档和真实实现不同，会导致错误修复方向。
...
- `projects/ChatAIO/docs/architecture/ai-config.md`
- `projects/ChatAIO/docs/architecture/appearance-theme-environment.md`
- `projects/ChatAIO/docs/architecture/i18n.md`
- `projects/ChatAIO/docs/issues/*.md`
- `projects/ChatAIO/todo.md`
- 本文件 `projects/ChatAIO/fixme.md`
