...
AI-WebApp 当前打包构建流程可运行，但架构边界不够清晰：公共 `engine/webpack` 承担了 webpack、WebpackDevServer、React Refresh、HTTPS 证书、HMR、构建落盘等底层细节；AI-WebApp 的 `partial.webpack-conf.ts` 又重复手写 renderer 多入口、HTML 插件、输出路径和运行时 URL。这个组合导致 View 入口变更、dev server fallback、HMR public path 和 Electron load URL 之间缺少单一事实来源。

本次改造目标不是重写构建系统，而是增加一层较薄的应用入口抽象：

- AI-WebApp 只声明 renderer entry manifest。
...
- 完整阅读 monorepo 公共结构、AI-WebApp 子工程代码和文档，尤其是 `.qoder` / `.codex` / `.cursor` 中面向 agent 的规则。
...
- AI-WebApp 文档：`todo.md`、`fixme.md`、`docs/architecture/*`、`docs/features/*`、`docs/issues/*`。
- 构建入口：`engine/webpack/*`、`scripts/webpack.start/*`、`scripts/webpack.build/*`、`scripts/electron.start/*`、`scripts/utils/*`。
- AI-WebApp runtime：`partial.webpack-conf.ts`、`src/Main/reaxels/Views/*`、`src/Main/services/dev/*`、`src/Views/*`。

根目录 `AGENTS.md` 和 skill 中提到的 `projects/AI-WebApp/AI-CONFIG-ARCHITECTURE.md` 在当前工作区不存在；本次以用户消息中的 AGENTS 规则和现存仓库文件为准。

## 技术调研结论

### Webpack 多入口和 HMR

AI-WebApp renderer 输出为多入口目录：
...
全量 reload 时，当前公共 dev server 的 `historyApiFallback.index = '/renderer/index.html'` 与 AI-WebApp 实际输出不匹配。AI-WebApp 没有 `renderer/index.html`，因此在某些刷新或 fallback 场景下会出现 Electron 里 load 失败，只能重启 webpack 和 Electron 恢复。
...
- AI-WebApp 把 Main、Views、Settings services 分层，整体仍能沿 Reaxes/Electron 边界扩展。

### 脆弱部分

- 公共 dev server fallback 指向不存在的 `/renderer/index.html`。
- development renderer 没有显式 `publicPath`，多入口子目录下 HMR 资源定位不稳定。
- AI-WebApp 的 renderer entry、HTML 插件和 Main load URL 分散重复。
...
公共 dev server 是 monorepo 共享配置，因此 fallback 不能只服务 AI-WebApp。已核对其他现有 Electron 子工程的 renderer 输出形态：
...
新的 fallback 策略保留根 `/index.html`，并把首段路径形式的入口 fallback 到 `/<entry>/index.html`。这覆盖 AI-WebApp 的本地 View，也不要求其他子工程改动现有 URL。

## 架构方案

### 1. Renderer Entry Manifest

新增 AI-WebApp 共享 manifest：
...
- 将 `HtmlWebpackPlugin` 这种底层细节留在公共构建层，AI-WebApp 只声明“有哪些 renderer entry”。

### 3. Runtime Renderer URL Helper

新增 AI-WebApp Main 侧工具：
...
- AI-WebApp `partial.webpack-conf.ts` 不再重复手写 4 个 HtmlWebpackPlugin。
- Main 侧不再重复定义 renderer dev URL helper。
- `tsc -p projects/AI-WebApp/tsconfig.json --noEmit` 不因本次改动新增错误；如遇历史 blocker，需要记录。
...
2. **Dev server fallback（`engine/webpack/devserver.ts`）**  
   全量 reload 或 `webContents.reload()` 时，dev server 的 `historyApiFallback.index` 原先指向 `/renderer/index.html`，而 AI-WebApp 并无此文件 → Electron 内页面 404，只能重启 webpack + Electron。
