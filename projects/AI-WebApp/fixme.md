...
## P0-01 TypeScript 检查当前不可用

### 问题现状

AI-WebApp 根 tsconfig 以及三个 renderer 子 tsconfig 都配置了过宽的 `typeRoots`：
...
实际运行以下命令时，TypeScript 会先把普通目录当成类型包扫描，导致 `TS2688 Cannot find type definition file`：
...
- `.\node_modules\.bin\tsc.cmd -p projects\AI-WebApp\src\Views\GuidingView\tsconfig.json --noEmit`
- `.\node_modules\.bin\tsc.cmd -p projects\AI-WebApp\src\Views\Floating-Layer\tsconfig.json --noEmit`

根 tsconfig 会报 `dist`、`docs`、`src`、`statics`、`__Bin` 等目录缺少类型定义；renderer tsconfig 会报 `components`、`reaxels`、`services` 等目录缺少类型定义。

### 为什么有问题以及后果

AI-WebApp 的核心边界是 Electron IPC、settings schema、AI page lifecycle、proxy config。这些边界如果没有类型检查兜底，会让错误更晚暴露到运行时。

当前 tsc 无法进入真实类型诊断阶段，因此无法知道项目内还有多少实际类型错误。后续修改功能时，即使引入真实类型回归，也可能被现有 `typeRoots` 噪声掩盖。
...
1. 移除把项目根目录或 view 根目录放入 `typeRoots` 的配置。
2. 只保留真实类型包目录，例如 `../../node_modules/@types` 或根部 `node_modules/@types`，具体相对路径按当前 tsconfig 位置计算。
3. 如果需要项目内 global 声明，继续通过 `include` 明确包含 `global.d.ts`、`g.d.ts`，不要通过 `typeRoots` 扫普通目录。
4. 统一检查四个 tsconfig 的 `paths` 与 `include`，删除无效 include，例如字面量 `#generics/**/*` 这种 TypeScript 不会按 path alias 展开的 include。
5. 修复 `typeRoots` 后再运行 tsc，记录真实类型错误并单独拆分后续修复项。

验证方式：

- 四个 `tsc --noEmit` 命令不再出现 `TS2688`。
- 如果出现新的真实类型错误，应能定位到具体源码文件，而不是目录扫描错误。
