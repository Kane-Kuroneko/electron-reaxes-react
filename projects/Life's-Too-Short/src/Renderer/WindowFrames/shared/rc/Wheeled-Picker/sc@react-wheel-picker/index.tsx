...
const WheelPicker: React.FC<WheelPickerProps> = ( {
	...
	const radius = itemHeight / Math.tan( (
		itemAngle * Math.PI
	) / 180 );
	...
	const handleWheelItemClick = ( clientY: number ) => {
		...
		const stepsToScroll = (
			quarterCount - clickedSegmentIndex - 1
		) * -1;
	...
	const finalizeDragAndStartInertiaScroll = () => {
		try {
			...
			if( yList.length > 1 ) {
				...
				const [ startY , startTime ] = yList[len - 2] ?? [
					0 ,
					0 ,
				];
				const [ endY , endTime ] = yList[len - 1] ?? [
					0 ,
					0 ,
				];
	...
	); // selected value is changed when options are changed
