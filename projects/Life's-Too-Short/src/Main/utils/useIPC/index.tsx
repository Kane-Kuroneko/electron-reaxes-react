export const IpcMainHandle = function () {
	...
	return <T extends keyof IpcJsonHandle>( type: T ) => {
		...
		return {
			handle( cb: ( e: IpcMainInvokeEvent , data: IpcJsonHandle[T]['data'] ) => IpcJsonHandle[T]['reply'] | Promise<IpcJsonHandle[T]['reply']> ) {
...
export const IpcMainOn = function () {
	...
	return <T extends keyof IpcJsonOn>( type: T ) => {
		...
		return {
			on( cb: ( e: IpcMainEvent , data: IpcJsonOn[T]['data'] , reply : ReturnType<typeof $reply>) => void ):void {
				registry[type] = cb as any;
			} ,
		};
	};
