...
class CreateAutoSave {
	__New( interval := 5*60*1000 ){
		this.StartTime := A_Now
		this.Timer := SetTimer(this.SaveGame.Bind(this),interval)
	}
	
	SaveGame(){
		if !WinActive(WarcraftIIIProcess)
			return
		Send("{RAlt DownR}{S}{RAlt Up}")
		Sleep(100)
		time := FormatTime(,"yyyy.M.d HH：mm")
		Send("AutoSave_" time "{Enter}")
	}
	Stop(){
	...
	ChangeInterval(NewInterval){
		this.__New(NewInterval)
		return this
	}
}

;AutoSaveInstance := CreateAutoSave(2*60*1000)

;AutoSaveInstance.ChangeInterval(10*60*1000).Restart()
...
;LAltTriggered := false
;LAlt::{
;	global LAltTriggered := true
;	return
;}
;LAlt Up::{
;	global LAltTriggered := false
;	return
;}
#HotIf WinActive(WarcraftIIIProcess) && G_MainSwitch
;;防止ctrl+shift+s关闭声音
